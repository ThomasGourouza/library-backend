...
import com.tgourouza.library_backend.constant.BookTag;
...
public class TagsMapper {
    ...
    TagsMapper() {
        tagMap.put(BookTag.LITERATURE.getValue(), new HashSet<>(Arrays.asList(
    ...
    public List<String> fromList(List<String> tags) {
        ...
        // collect tags
        List<String> result = tagMap.entrySet().stream()
        ...
        // refine science rule
        if (result.contains(BookTag.SCIENCE.getValue())) {
            ...
            boolean hasPureScience = normalized.stream().anyMatch(
                    phrase -> tagMap.get(BookTag.SCIENCE.getValue()).stream()
            ...
            if (hasExclusion && !hasPureScience) {
                result.remove(BookTag.SCIENCE.getValue());
            }
        }
